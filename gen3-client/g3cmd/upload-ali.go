...
func initAli() {
	...
	var uploadAliCmd = &cobra.Command{
		Use:   "upload-ali",
		Short: "Upload file(s) to object storage and update indexd record.",
		Long:  `Gets a presigned URL for each file and then uploads the specified file(s).`,
		Example: "For uploading a single file:\n./gen3-client upload-ali --profile=<profile-name> --upload-path=<path-to-files/data.bam>\n" +
			"For uploading all files within an folder:\n./gen3-client upload-ali --profile=<profile-name> --upload-path=<path-to-files/folder/>\n" +
			"Can also support regex such as:\n./gen3-client upload-ali --profile=<profile-name> --upload-path=<path-to-files/folder/*>\n" +
			"Or:\n./gen3-client upload-ali --profile=<profile-name> --upload-path=<path-to-files/*/folder/*.bam>\n" +
			...
			if batch {
				...
				for _, filePath := range singlepartFilePaths {
					...
					if err != nil {
						...
						err = UpdateIndexdRecord(gen3Interface, furObject.GUID, furObject.FilePath)
						if err != nil {
							log.Printf("Error updating indexd record for %s: %s", furObject.GUID, err.Error())
						}
					}
					file.Close()
				}
	...
	uploadAliCmd.Flags().StringVar(&profile, "profile", "", "Specify profile to use")
	uploadAliCmd.MarkFlagRequired("profile") //nolint:errcheck
	uploadAliCmd.Flags().StringVar(&uploadPath, "upload-path", "", "The directory or file in which contains file(s) to be uploaded")
	uploadAliCmd.MarkFlagRequired("upload-path") //nolint:errcheck
	uploadAliCmd.Flags().BoolVar(&batch, "batch", false, "Upload in parallel")
	uploadAliCmd.Flags().IntVar(&numParallel, "numparallel", 3, "Number of uploads to run in parallel")
	uploadAliCmd.Flags().BoolVar(&includeSubDirName, "include-subdirname", false, "Include subdirectory names in file name")
	uploadAliCmd.Flags().BoolVar(&forceMultipart, "force-multipart", false, "Force to use multipart upload if possible")
	uploadAliCmd.Flags().BoolVar(&hasMetadata, "metadata", false, "Search for and upload file metadata alongside the file")
	uploadAliCmd.Flags().StringVar(&bucketName, "bucket", "", "The bucket to which files will be uploaded. If not provided, defaults to Gen3's configured DATA_UPLOAD_BUCKET.")
	RootCmd.AddCommand(uploadAliCmd)
}
